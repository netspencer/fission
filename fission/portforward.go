...
func runportForward(serviceName string, localPort string) error {
	...
	// check if FISSION_NAMESPACE env var is defined and use it.
	namespace := ""
	...
	podList, err := podClient.CoreV1().Pods(namespace).List(meta_v1.ListOptions{LabelSelector: "application=fission-api"})
	if err != nil || len(podList.Items) == 0 {
		fatal("Error getting controller pod for port-forwarding")
	}

	// if there are more than one pods, always choose the first pod returned to port-forward to
...
func controllerPodPortForward() string {
